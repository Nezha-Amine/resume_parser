...
const ResumeBuilder = () => {
  ...
  const handleDownloadPdf = () => {
    ...
    html2canvas(rightSideContent, {
      scale: 2, // Increase the scale for better quality
      dpi: 300, // Set the DPI (dots per inch) for higher resolution
      useCORS: true, // Use CORS to load external images correctly
    }).then((canvas) => {
      const imgData = canvas.toDataURL("image/png");
  
      // Create a PDF with proper page dimensions
      const pdf = new jsPDF({
